...
The `Weapon weight` row in `VR SETTINGS` cycles through four presets:

| Preset | Profile multiplier |
|---|---:|
| `NONE` | `0x` (disabled) |
| `LIGHT` | `0.5x` |
| `MEDIUM` | `1x` |
...
The selection is saved as `WeaponWeightPreset=0`, `1`, `2`, or `3` under
`[VR]`. Existing configurations that only contain `WeaponWeightEnabled=1`
are loaded as `MEDIUM` for backward compatibility.
...
Select `NONE` in the VR menu or set `WeaponWeightPreset=0` to disable
simulated weapon weight.
